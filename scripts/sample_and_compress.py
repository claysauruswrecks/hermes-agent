...
def sample_from_datasets(
    ...
    with Pool(
        ...
        for result in pool.imap_unordered(
            _count_tokens_for_entry, all_entries, chunksize=100
        ):
    ...
    if token_counts:
        ...
        print(
            f"   📈 Token stats: min={min(token_counts):,}, max={max(token_counts):,}, avg={avg_tokens:,.0f}"
        )
...
def save_samples_for_compression(
    samples: List[Dict[str, Any]], output_dir: Path, batch_size: int = 100
):
    """
