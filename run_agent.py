...
class AIAgent:
    ...
    def _credential_pool_may_recover_rate_limit(self) -> bool:
        ...
        if pool is None:
            return False
        if str(getattr(self, "base_url", "")).startswith("cloudcode-pa://"):
    ...
    def _fire_stream_delta(self, text: str) -> None:
        ...
        # Capture reasoning/thinking text for TTS playback if enabled
        _captured_reasoning_text = None
        if isinstance(text, str):
            ...
            if think_scrubber is not None:
                # Capture reasoning/thinking text for TTS playback if enabled
                # Check if tts_reasoning is configured to capture reasoning
                capture_reasoning_for_tts = False
                try:
                    from hermes_cli.config import load_config as _lc
                    full_config = _lc()
                    capture_reasoning_for_tts = full_config.get("tts_reasoning", {}).get("enabled", False)
                except Exception:
                    capture_reasoning_for_tts = False
                
                if capture_reasoning_for_tts and hasattr(think_scrubber, '_captured_reasoning'):
                    # Capture any reasoning text that was held in the scrubber
                    captured_text = "".join(think_scrubber._captured_reasoning) if think_scrubber._captured_reasoning else None
                    if captured_text and captured_text.strip():
                        _captured_reasoning_text = captured_text
                
                # Feed to the scrubber (which discards reasoning text from display output)
        ...
        if not text:
            # If no visible text but we have captured reasoning text for TTS, 
            # fire the reasoning delta callback for TTS playback
            if '_captured_reasoning_text' in locals() and _captured_reasoning_text:
                self._fire_reasoning_delta(_captured_reasoning_text)
            return
    ...
    def _fire_reasoning_delta(self, text: str) -> None:
        """Fire reasoning callback if registered and trigger TTS playback if enabled."""
        # Check if tts_reasoning is enabled in config
        ...
        # If tts_reasoning is enabled, trigger TTS audio playback for reasoning text
        if tts_reasoning_enabled and text and text.strip():
            try:
                ...
                # Truncate very long text for TTS (4000 char cap similar to regular voice mode)
                tts_text = text[:4000] if len(text) > 4000 else text
                
                # Generate audio file using synchronous TTS with reasoning_mode=True
                os.makedirs(os.path.join(tempfile.gettempdir(), "hermes_voice"), exist_ok=True)
                mp3_path = os.path.join(
                    tempfile.gettempdir(),
                    "hermes_voice",
                    f"reasoning_tts_{time.strftime('%Y%m%d_%H%M%S')}.mp3",
                )

                _debug_reasoning_tts = f"speak_reasoning: synthesizing {len(tts_text)} chars -> {mp3_path}"
                logger.debug(_debug_reasoning_tts)
                
                text_to_speech_tool(text=tts_text, output_path=mp3_path, reasoning_mode=True)

                if os.path.isfile(mp3_path) and os.path.getsize(mp3_path) > 0:
                    _debug_play = f"speak_reasoning: playing {mp3_path} ({os.path.getsize(mp3_path)} bytes)"
                    logger.debug(_debug_play)
                    ...
                    logger.debug(f"speak_reasoning: TTS tool produced no audio at {mp3_path}")
                    
            except Exception:
                # TTS playback failure should not break reasoning callback or agent loop
                logger.debug("Reasoning TTS playback failed")
    ...
    def _fire_tool_gen_started(self, tool_name: str) -> None:
        ...
        Also triggers TTS playback for tool-use streams if tts_tool_use.enabled is true.
        """
        ...
        # If tts_tool_use is enabled, trigger TTS audio playback for tool generation context
        ...
        if tts_tool_use_enabled and not hasattr(self, "_tool_use_tts_initiated"):
            try:
                # Mark as initiated to avoid multiple TTS startups for the same tool turn
                self._tool_use_tts_initiated = True
            except Exception:
                # Tool-use TTS initialization failure should not break the main callback or agent loop
                logger.debug("Tool-use TTS init failed: %s", Exception())

        if tts_tool_use_enabled and (
            hasattr(self, "_tool_use_tts_initiated") or tool_name is not None
        ):
            try:
                ...
                # Get or create the tool use tts queue and events
                if not hasattr(self, "_tool_use_tts_queue"):
                    self._tool_use_tts_queue = queue.Queue()
                    self._tool_use_tts_stop_event = threading.Event()
                    self._tool_use_tts_done_event = threading.Event()

                # Put tool name in the queue for streaming TTS
                if tool_name and not isinstance(tool_name, (dict, list, tuple)):
                    self._tool_use_tts_queue.put(f"Tool: {tool_name}")
                else:
                    self._tool_use_tts_queue.put("Generating tool output...")

                # Start TTS thread if not already running
                if (
                    not hasattr(self, "_tool_use_tts_thread")
                    or not self._tool_use_tts_thread.is_alive()
                ):

                    def display_callback_sentence(sentence: str):
                        """Called by TTS consumer when a sentence is ready to display + speak."""
                        pass  # Tool generation context already displayed via tool_gen_callback

                    tts_thread = threading.Thread(
                        target=stream_tts_to_speaker,
                        args=(
                            self._tool_use_tts_queue,
                            self._tool_use_tts_stop_event,
                            self._tool_use_tts_done_event,
                        ),
                        kwargs={
                            "display_callback": display_callback_sentence,
                            "tool_use_mode": True,
                        },
                        daemon=True,
                    )
                    tts_thread.start()
                    self._tool_use_tts_thread = tts_thread
            except Exception:
                # TTS playback failure should not break tool_gen callback or agent loop
                logger.debug("Tool-use TTS playback failed: %s", Exception())
