...
def _cmd_status(_args: argparse.Namespace) -> int:
    ...
    print(
        f"  sidecar deps        : {'✓ installed' if sidecar_installed else '✗ run `hermes photon install-sidecar`'}"
    )
    print(
        f"  telemetry           : {'on' if _telemetry_enabled() else 'off'} (`hermes photon telemetry on|off`)"
    )
    return 0
...
def _cmd_telemetry(args: argparse.Namespace) -> int:
    ...
    if state is None:
        ...
        print(
            "  Toggle with `hermes photon telemetry on` / `hermes photon telemetry off`."
        )
        return 0
    try:
    ...
    print(
        "  Restart the gateway for the sidecar to pick it up:  hermes gateway restart"
    )
    return 0
