...
class DiscordAdapter(BasePlatformAdapter):
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        try:
            ...
            async def on_message(message: DiscordMessage):
                ...
                if (
                    ...
                    if (
                        ...
                        if "*" not in _free_channels and not (
                            _channel_ids & _free_channels
                        ):
                            return
            ...
            await _wait_for_ready_or_bot_exit(
                self._ready_event, self._bot_task, timeout=30
            )
        ...
        while self._running:
            ...
            try:
                ...
                if fails < threshold:
                    continue
                logger.error(
                    "[%s] Discord client appears dead, forcing reconnect",
                    self.name,
                )
                try:
                    await client.close()
                    ...
                    logger.debug(
                        "[%s] Error closing wedged Discord client",
                        self.name,
                        exc_info=True,
                    )
    ...
    async def _handle_message(
        self, message: DiscordMessage, role_authorized: bool = False
    ) -> None:
    ...
    if existing:
        ...
        if not prompt_yes_no("Reconfigure Discord?", False):
            if not get_env_value("DISCORD_ALLOWED_USERS"):
                print_info(
                    "⚠️  Discord has no user allowlist - anyone can use your bot!"
                )
