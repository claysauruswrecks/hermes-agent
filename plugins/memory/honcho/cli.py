...
def cmd_setup(args) -> None:
    ...
    if not is_local:
        ...
        if existing_oauth is not None:
            print(
                f"    (currently connected via OAuth — client {existing_oauth.client_id})"
            )
        ...
        if use_oauth:
            ...
            try:
                ...
                print(
                    "  Re-run 'hermes honcho setup' to retry, or choose an API key instead.\n"
                )
                return
            ...
            new_key = _prompt(
                "Honcho API key (leave blank to keep current)", secret=True
            )
            if new_key:
                cfg["apiKey"] = new_key
    ...
    if gw_platforms is None:
        ...
        run_mapping = _prompt(
            "Configure gateway mapping anyway? (y/N)",
            default="n",
        ).strip().lower() in {"y", "yes"}
    else:
    ...
    if run_mapping:
        ...
        default_choice = {"single": "1", "hybrid": "2", "multi": "3"}.get(
            current_shape, "3"
        )
        ...
        if choice in {"2", "me+others", "both"}:
            pooled = (
                _prompt(
                    "  Keep my own memory pooled across platforms? (Y/n)",
                    default="y",
                )
                .strip()
                .lower()
        ...
        if shape == "single":
            ...
            print(
                f"  All non-agent gateway users route to '{peer_target}' (pin overrides aliases)."
            )
    ...
    current_dialectic = str(
        hermes_host.get("dialecticCadence") or cfg.get("dialecticCadence") or "2"
    )
    print("\n  Dialectic cadence:")
...
def cmd_status(args) -> None:
    ...
    host_block = (getattr(hcfg, "raw", None) or {}).get("hosts", {}).get(
        hcfg.host
    ) or {}
