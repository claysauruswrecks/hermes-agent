...
def build_oss_config(flags: dict[str, str]) -> tuple[dict, dict[str, str]]:
    ...
    if "default_url" in llm_def:
        llm_config["ollama_base_url"] = (
            flags.get("oss_llm_url") or llm_def["default_url"]
        )
    ...
    if "default_url" in embedder_def:
        embedder_config["ollama_base_url"] = (
            flags.get("oss_embedder_url") or embedder_def["default_url"]
        )
...
def _setup_oss_interactive(hermes_home: str, config: dict) -> None:
    ...
    if embedder_def["needs_key"] and embedder_id != llm_id:
        key = _prompt_api_key(
            f"{embedder_def['label']} embedder", embedder_def["env_var"], hermes_home
        )
        if key:
