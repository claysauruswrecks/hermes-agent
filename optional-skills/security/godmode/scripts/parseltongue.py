...
def _apply_leetheavy(word):
    ...
    return "".join(
        LEET_MAP_HEAVY.get(c.lower(), LEET_MAP.get(c.lower(), c)) for c in word
    )
...
TECHNIQUES = [
    ...
    {"name": "acrostic", "label": "Acrostic", "tier": 3, "fn": _apply_acrostic},
    {
        ...
        "label": "Dot+Uni",
        "tier": 3,
        "fn": _apply_dottedunicode,
    },
    {
        "name": "fullwidthmixed",
        "label": "FW MiX",
        "tier": 3,
        "fn": _apply_fullwidthmixed,
    },
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--escalate", type=int, default=None, help="Encoding escalation level (0-4)"
    )
    ...
    if args.escalate is not None:
        ...
        variants = generate_variants(
            args.query, tier=args.tier, custom_triggers=args.triggers
        )
        for v in variants:
