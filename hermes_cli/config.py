...
DEFAULT_CONFIG = {
    ...
    "auxiliary": {
        "_global_timeout": 30,  # seconds - global fallback timeout for any auxiliary task not explicitly configured
        "vision": {
        ...
        # Goal-judge — asks the auxiliary model whether an active /goal is
        # satisfied by the assistant's last response. Uses main chat model
        # (cache-safe side LLM call), but can route to a cheaper aux model
        # (e.g. openrouter google/gemini-3-flash-preview). Timeout should be
        # generous for slow local models; raise from default 30s as needed.
        "goal_judge": {
            "provider": "auto",
            "model": "",
            "base_url": "",
