...
def run_gui_uninstall(args):
    ...
    print(
        color(
            "This removes the Chat GUI only. The Hermes agent stays installed.",
            Colors.CYAN,
        )
    )
    print()
...
def run_uninstall(args):
    ...
    print(
        f"  Data:    {hermes_home / 'cron/'}, {hermes_home / 'sessions/'}, {hermes_home / 'logs/'}"
    )
    print()
    ...
    if full_uninstall:
        print(
            color(
                "⚠️  WARNING: This will permanently delete ALL Hermes data!",
                Colors.RED,
                Colors.BOLD,
            )
        )
    ...
    *,
    ...
    try:
        if project_root.exists():
            ...
            if (
                hermes_home in project_root.parents
                or project_root.parent == hermes_home
            ):
