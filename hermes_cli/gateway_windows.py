    ...
    *,
    ...
    try:
        ...
        if start_on_login is not None:
            os.environ["HERMES_GATEWAY_INSTALL_START_ON_LOGIN"] = (
...
def _install_startup_fallback(script_path: Path, start_now: bool, detail: str) -> None:
    ...
    if running_pids:
        ...
        profile_arg = _profile_arg()
        start_cmd = (
            f"hermes {profile_arg} gateway start"
            if profile_arg
        ...
        if prompt_yes_no("  Open the UAC prompt now?", False):
            if _launch_elevated_install(
                ...
                if start_now:
                    ...
                    print(
                        "  Approve the Windows UAC prompt, then run: hermes gateway status"
                    )
                return
            print(
...
def start() -> None:
    ...
    if not task_installed and not startup_installed:
        ...
        if not task_installed and not startup_installed:
            ...
            print(
                "  If a UAC prompt opened, approve it, then run: hermes gateway start"
            )
            return
