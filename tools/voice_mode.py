...
def detect_audio_environment() -> dict:
    ...
    if any(os.environ.get(v) for v in ("SSH_CLIENT", "SSH_TTY", "SSH_CONNECTION")):
        if has_forwarded_audio:
            notices.append(
                "Running over SSH with a reachable PulseAudio/PipeWire sound server"
            )
        else:
            warnings.append(
    ...
    if is_container():
        if has_forwarded_audio:
            notices.append(
                "Running inside container (Docker/Podman/LXC) with host audio forwarding"
            )
        else:
            warnings.append(
...
def transcribe_recording(wav_path: str, model: Optional[str] = None) -> Dict[str, Any]:
    ...
    if _should_chunk_for_transcription(wav_path, MAX_FILE_SIZE):
        result = _transcribe_wav_in_chunks(
            wav_path, model=model, max_file_size=MAX_FILE_SIZE
        )
    else:
...
def _transcribe_wav_in_chunks(
    ...
    *,
    ...
    try:
        chunk_paths = _split_wav_for_transcription(
            wav_path, max_file_size=max_file_size
        )
        if not chunk_paths:
            return {
                "success": False,
        ...
        logger.info(
            "Transcribing oversized WAV in %d chunks: %s", len(chunk_paths), wav_path
        )
...
def play_audio_file(file_path: str) -> bool:
    ...
    def _watch_playback_proc(proc):
        """Background watchdog for playback process cleanup."""
        try:
            proc.wait(timeout=300)
        except subprocess.TimeoutExpired:
            logger.warning("System player %s timed out, killing process", proc.args[0] if proc.args else "unknown")
            try:
                proc.kill()
                proc.wait()
            except Exception:
    ...
    for cmd in players:
        ...
        if exe:
            try:
                ...
                # Run watchdog in background thread to avoid blocking next inference chunk
                watch_thread = threading.Thread(target=_watch_playback_proc, args=(proc,), daemon=True)
                watch_thread.start()
