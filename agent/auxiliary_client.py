...
def _is_codex_gpt55(model: Optional[str], provider: Optional[str] = None) -> bool:
    ...
    bare = (model or "").strip().lower().rsplit("/", 1)[-1]
    return (
        bare == "gpt-5.5" or bare.startswith("gpt-5.5-") or bare.startswith("gpt-5.5.")
    )
...
class _AnthropicCompletionsAdapter:
    ...
    def create(self, **kwargs) -> Any:
        ...
        if _skip_mt:
            ...
            max_tokens = (
                kwargs.get("max_tokens") or kwargs.get("max_completion_tokens") or 2000
            )
    ...
    for i, entry in enumerate(chain):
        ...
        if fb_client is not None:
            ...
            logger.info(
                ...
                failed_provider,
                label,
                resolved_model or fb_model or "default",
            )
...
def _try_main_fallback_chain(
    ...
    try:
        ...
        logger.debug(
            "Auxiliary %s: could not load main fallback chain: %s", task or "call", exc
        )
        return None, None, ""
...
def auxiliary_max_tokens_param(value: int, *, model: Optional[str] = None) -> dict:
    ...
    if (
        ...
        and base_url_hostname(custom_base)
        in {"api.openai.com", "api.githubcopilot.com"}
    ):
...
def _cached_client_accepts_slash_models(
    client: Any, cached_default: Optional[str]
) -> bool:
...
def _compat_model(
    client: Any, model: Optional[str], cached_default: Optional[str]
) -> Optional[str]:
...
def _get_task_timeout(task: str, default: float = _DEFAULT_AUX_TIMEOUT) -> float:
    """Read timeout from auxiliary.{task}.timeout in config, falling back to global fallback or *default*."""
    if not task:
        return default
    task_config = _get_auxiliary_task_config(task)
    ...
    # Fallback to global auxiliary timeout if configured
    try:
        from hermes_cli.config import load_config

        cfg = load_config()
        aux_cfg = cfg.get("auxiliary", {}) or {}
        raw_global = aux_cfg.get("_global_timeout")
        if raw_global is not None:
            try:
                val = float(raw_global)
                if val > 0:
                    return val
            except (ValueError, TypeError):
                pass
...
def _build_call_kwargs(
    ...
    if max_tokens is not None:
        ...
        if _is_anthropic_compat_endpoint(provider, _effective_base) or _is_nvidia_nim:
...
def call_llm(
    ...
    *,
    ...
    # Log what we're about to do — makes auxiliary operations visible, including the resolved timeout
    _base_info = str(getattr(client, "base_url", resolved_base_url) or "")
    logger.info(
        "Auxiliary %s: using %s (%s), timeout=%.1fs%s",
        task,
