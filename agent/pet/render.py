...
def _raw_frames(
    ...
    try:
        ...
        logger.debug(
            "pet frame decode failed (%s, %s): %s", sheet_path, state_value, exc
        )
        return ()
...
def state_frame_counts(
    ...
    *,
    ...
    return {
        state.value: len(
            _raw_frames(
                str(sheet_path), state.value, frame_w, frame_h, frames_per_state
            )
        )
...
def _encode_kitty(
    frame, *, cell_cols: int | None = None, cell_rows: int | None = None
) -> str:
    ...
    if cell_rows:
        ctrl += f",r={cell_rows}"
    return _kitty_apc(
        ctrl, base64.standard_b64encode(_png_bytes(frame)).decode("ascii")
    )
...
def _encode_sixel(frame) -> str:
    ...
    for idx in used:
        ...
        out.append(
            "#%d;2;%d;%d;%d" % (idx, r * 100 // 255, g * 100 // 255, b * 100 // 255)
        )
...
class PetRenderer:
    ...
    def cells(
        self, state: PetState | str, index: int, *, cols: int | None = None
    ) -> list[list[Cell]]:
    ...
    def kitty_payload(self, state: PetState | str, *, image_id: int) -> dict | None:
        ...
        return {
            ...
            "frames": [
                _encode_kitty_virtual(f, image_id=image_id, cols=cols, rows=rows)
                for f in frames
            ],
        }
