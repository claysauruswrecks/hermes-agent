...
def init_agent(
    ...
    # Check if tts_reasoning is enabled to configure scrubber for TTS playback
    ...
    agent._stream_think_scrubber = StreamingThinkScrubber(capture_reasoning_for_tts=capture_reasoning_for_tts)
    ...
    if not agent.quiet_mode:
        if compression_enabled:
            print(
                f"📊 Context limit: {agent.context_compressor.context_length:,} tokens (compress at {int(compression_threshold * 100)}% = {agent.context_compressor.threshold_tokens:,})"
            )
        else:
            print(
