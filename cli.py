...
def _setup_worktree(
    repo_root: str = None, sync_base: bool = True
...
class HermesCLI(CLIAgentSetupMixin, CLICommandsMixin):
    ...
    def __init__(
        ...
        self._last_turn_finished_at: Optional[float] = (
            None  # time.time() when the last agent loop finished
        )
    ...
    def _claim_active_session(
        self, surface: str = "cli", *, stderr: bool = False
    ) -> bool:
    ...
    def _invalidate(self, min_interval: float = 0.25) -> None:
        ...
        if (
            hasattr(self, "_app")
            and self._app
            and (now - getattr(self, "_last_invalidate", 0.0)) >= min_interval
        ):
    ...
    def _pet_frames_for(self, state: str) -> list:
        ...
        try:
            ...
            grids = [
                renderer.cells(state, i, cols=self._pet_cols) for i in range(count)
            ]
        except Exception:
            grids = []
        ...
        for y, row in enumerate(grid):
            ...
            for top, bottom in row:
                ...
                if not top_op and not bot_op:
                    ...
                    frags.append((
                        f"fg:#{tr:02x}{tg:02x}{tb:02x} bg:#{br:02x}{bg:02x}{bb:02x}",
                        "▀",
                    ))
                elif top_op:
    ...
    def _pet_start_anim(self) -> None:
        ...
        self._pet_anim_thread = threading.Thread(
            target=self._pet_anim_loop, daemon=True
        )
    ...
    def _submit_editor_buffer(self, buffer) -> None:
        ...
        if self._agent_running:
            ...
            self._interrupt_queue.put(
                text
            ) if self.busy_input_mode == "interrupt" else self._pending_input.put(text)
        ...
        if canonical in {"quit", "exit"}:
            ...
            try:
                ...
                if not user_entries:
                    ...
                    print(
                        f"  Or drop a plugin directory into {display_hermes_home()}/plugins/"
                    )
                    if bundled_count:
                        print(
            ...
            _cprint(
                f"  MoA one-shot queued with preset {preset}; previous model will be restored after this turn."
            )
    ...
    def _billing_overview(self, state):
        ...
        if cap is not None and cap.limit_usd is not None:
            ...
            bar, pct = self._billing_spend_bar(cap.spent_this_month_usd, cap.limit_usd)
        ...
        if not state.cli_billing_enabled:
            ...
            self._billing_portal_hint(
                state, reason="Enable it on the portal to buy credits here."
            )
            return
    ...
    def _billing_poll_charge(self, state, charge_id, amount):
        ...
        print(
            f"  🟡 Still processing after 5 minutes — this is a timeout, not a "
    ...
    def _billing_render_charge_failed(self, state, reason):
        ...
        if reason == "authentication_required":
            print(
                "  🔴 Your bank requires verification (3DS). Complete it on the "
            ...
            print(
                f"  🔴 The charge didn't go through ({reason or 'processing_error'})."
            )
        self._billing_portal_hint(state)
    ...
    def _billing_render_charge_error(self, state, exc):
        ...
        portal_url = getattr(exc, "portal_url", None) or getattr(
            state, "portal_url", None
        )
        if code == "no_payment_method":
            print(
                "  💳 No saved card for terminal charges yet. Set one up on the "
            ...
            print(
                f"  🟡 Too many charges right now{mins}. This isn't a payment failure."
            )
        else:
            print(f"  🔴 {exc}")
        if portal_url:
    ...
    def _billing_handle_scope_required(self, state):
        ...
        _scope_msg = (
            'An org admin/owner must tick "Allow terminal billing" during login.'
        )
        ...
        if granted:
            ...
            if fresh.logged_in and fresh.cli_billing_enabled:
                ...
                print(
                    "  🟡 Permission granted, but terminal billing is still turned "
    ...
    def _billing_auto_reload_flow(self, state):
        ...
        try:
            patch_auto_top_up(
                enabled=True,
                threshold=float(threshold_amt),
                top_up_amount=float(reload_amt),
            )
        except BillingScopeRequired:
            self._billing_handle_scope_required(state)
            return
            ...
            return
        print(
            f"  ✅ Auto-reload on: below {format_money(threshold_amt)} → "
    ...
    def _billing_limit_screen(self, state):
        ...
        _limit_note = (
            "The monthly limit is set on the portal — the terminal shows it read-only."
        )
    ...
    def _on_tool_complete(
        ...
        if function_name == "delegate_task":
            ...
            if (
                ...
                noun, tail = (
                    ("task", "it finishes") if n == 1 else (f"{n} tasks", "they finish")
                )
                try:
                    _cprint(
    ...
    def _show_voice_status(self):
        ...
        # Show TTS provider and reasoning mode status if voice/tts is enabled
        try:
            from tools.tts_tool import (
                _load_tts_config,
                _get_provider,
                _load_tts_reasoning_config as _load_tts_rc,
            )

            tts_cfg = _load_tts_config()
            provider = _get_provider(tts_cfg)
            _cprint(f"  TTS Provider: {provider}")

            # Check if tts_reasoning is enabled
            try:
                from hermes_cli.config import load_config as _lc

                full_cfg = _lc()
                tts_reasoning_cfg = full_cfg.get("tts_reasoning", {}) or {}
                reasoning_enabled = bool(tts_reasoning_cfg.get("enabled", False))
                # More accurate check: if 'provider' key exists or if section has provider settings
                has_provider_settings = "provider" in tts_reasoning_cfg or any(
                    k in tts_reasoning_cfg
                    for k in [
                        "edge",
                        "elevenlabs",
                        "openai",
                ...
                reasoning_active = reasoning_enabled or has_provider_settings

                if reasoning_active:
                    reason_prov = tts_reasoning_cfg.get(
                        "provider", "default (inherits from main TTS)"
                    )
                    _cprint(f"  Reasoning TTS: ENABLED (provider: {reason_prov})")
                else:
                    _cprint(f"  Reasoning TTS: DISABLED")
            except Exception:
                pass

            # Check if tts_tool_use is enabled
            try:
                from hermes_cli.config import load_config as _lc

                full_cfg = _lc()
                tts_tool_use_cfg = full_cfg.get("tts_tool_use", {}) or {}
                tool_use_enabled = bool(tts_tool_use_cfg.get("enabled", False))
                # More accurate check: if 'provider' key exists or if section has provider settings
                has_provider_settings = "provider" in tts_tool_use_cfg or any(
                    k in tts_tool_use_cfg
                    for k in [
                        "edge",
                        "elevenlabs",
                        "openai",
                ...
                tool_use_active = tool_use_enabled or has_provider_settings

                if tool_use_active:
                    tool_prov = tts_tool_use_cfg.get(
                        "provider", "default (inherits from main TTS)"
                    )
                    _cprint(f"  Tool-use TTS: ENABLED (provider: {tool_prov})")
                else:
                    _cprint(f"  Tool-use TTS: DISABLED")
            except Exception:
                pass
            ...
            def run_agent():
                ...
                try:
                    ...
                    if getattr(self, "_pending_moa_disable_after_turn", False):
                        _restore = (
                            getattr(self, "_pending_moa_restore_model", None) or {}
                        )
    ...
    def run(self):
        ...
        status_bar = ConditionalContainer(
            ...
            filter=Condition(
                lambda: (
                    cli_ref._status_bar_visible
                    and not getattr(
                        cli_ref, "_status_bar_suppressed_after_resize", False
                    )
                )
            ),
        )
        ...
        _cpr_disabled_output = (
            _build_cpr_disabled_output(sys.stdout) if _terminal_may_leak_cpr() else None
        )
