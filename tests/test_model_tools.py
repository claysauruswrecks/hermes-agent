...
class TestHandleFunctionCall:
    ...
    def test_tool_request_and_execution_middleware_wrap_registry_dispatch(
        self, monkeypatch
    ):
        seen = {}
        ...
        monkeypatch.setattr(
            "hermes_cli.plugins.invoke_middleware", fake_invoke_middleware
        )
...
class TestDisabledToolsetsPlatformBundle:
    ...
    def test_disabling_non_platform_toolset_still_works(self):
        ...
        assert present_web <= removed, f"Web tools not removed: {present_web - removed}"
    ...
    def test_disabling_bundle_removes_platform_tools_but_keeps_core(self):
        ...
        assert not (delta & set(_HERMES_CORE_TOOLS)), (
            "core tools must not be in the removal delta"
        )
