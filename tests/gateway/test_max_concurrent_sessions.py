...
def _silence_global_gateway_hooks(monkeypatch):
    ...
    monkeypatch.setattr(
        "tools.slash_confirm.clear_if_stale", lambda *args, **kwargs: None
    )
    monkeypatch.setattr(
        "tools.approval.has_blocking_approval", lambda *args, **kwargs: False
    )
...
def test_existing_active_session_uses_busy_handling_at_limit(monkeypatch):
    ...
    async def fail_if_agent_runs(self_inner, ev, src, qk, generation):
        raise AssertionError(
            "_handle_message_with_agent should not run for busy follow-up"
        )
