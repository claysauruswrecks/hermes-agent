"""Tests for Mattermost platform adapter reasoning transport mechanics."""
...
class TestMattermostReasoningTransportStreamingMode:
    """Verify reasoning tokens transport in Mattermost streaming mode."""

    @pytest.mark.asyncio
    async def test_mattermost_streaming_reasoning_callback_format(self):
        ...
        consumer = GatewayStreamConsumer(adapter, "channel_1", config)
    ...
    @pytest.mark.asyncio
    async def test_mattermost_streaming_reasoning_callback_with_regular_deltas(self):
...
class TestMattermostReasoningTransportNonStreamingMode:
    """Verify reasoning tokens transport in Mattermost non-streaming mode via thinking_progress."""

    def test_mattermost_thinking_progress_reasoning_format(self):
    ...
    def test_mattermost_verbose_reasoning_config_true(self):
        """When config.extra['verbose_reasoning'] is true, Mattermost adapter returns True."""
    ...
    def test_mattermost_verbose_reasoning_config_false(self):
        """When config.extra['verbose_reasoning'] is false, Mattermost adapter returns False."""
        adapter = MagicMock()
        adapter.config = MagicMock()
...
class TestMattermostReasoningContentParity:
