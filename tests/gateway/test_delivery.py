...
class StaleTopicAdapter:
    ...
    async def send(self, chat_id, content, metadata=None):
        ...
        if len(self.calls) == 1:
            return SendResult(
                success=False, error="Bad Request: message thread not found"
            )
...
@pytest.mark.asyncio
async def test_explicit_telegram_private_thread_requires_reply_anchor(
...
@pytest.mark.asyncio
async def test_named_telegram_private_topic_is_created_before_delivery(
    tmp_path, monkeypatch
):
    monkeypatch.setattr("gateway.delivery.get_hermes_home", lambda: tmp_path)
    adapter = RecordingAdapter()
...
@pytest.mark.asyncio
async def test_named_telegram_private_topic_refreshes_stale_thread_id(
    tmp_path, monkeypatch
):
    monkeypatch.setattr("gateway.delivery.get_hermes_home", lambda: tmp_path)
    ...
    assert [call["metadata"]["thread_id"] for call in adapter.calls] == [
        "32343",
        "38064",
    ]
    assert all(
        call["metadata"]["telegram_dm_topic_created_for_send"] is True
        for call in adapter.calls
    )


@pytest.mark.asyncio
async def test_explicit_telegram_private_thread_uses_reply_fallback_with_anchor(
    tmp_path, monkeypatch
):
    monkeypatch.setattr("gateway.delivery.get_hermes_home", lambda: tmp_path)
    adapter = RecordingAdapter()
...
@pytest.mark.asyncio
async def test_explicit_telegram_direct_messages_topic_metadata_is_respected(
    tmp_path, monkeypatch
):
    monkeypatch.setattr("gateway.delivery.get_hermes_home", lambda: tmp_path)
    adapter = RecordingAdapter()
...
@pytest.mark.asyncio
async def test_explicit_telegram_group_thread_does_not_mark_dm_fallback(
    tmp_path, monkeypatch
):
    monkeypatch.setattr("gateway.delivery.get_hermes_home", lambda: tmp_path)
    adapter = RecordingAdapter()
...
async def test_platform_send_failure_raises_for_delivery_result(tmp_path, monkeypatch):
    ...
    with pytest.raises(RuntimeError, match="route failed"):
        await router._deliver_to_platform(
            target, "hello", metadata={"telegram_reply_to_message_id": "9001"}
        )
...
async def test_short_output_never_truncated(tmp_path, monkeypatch):
    ...
    short_content = "x" * 100
    await router._deliver_to_platform(
        target, short_content, metadata={"job_id": "job3"}
    )
...
@pytest.mark.asyncio
async def test_audit_save_failure_does_not_break_chunking_delivery(
    tmp_path, monkeypatch
):
...
@pytest.mark.asyncio
async def test_save_failure_during_truncation_raises_for_non_chunking_adapter(
    tmp_path, monkeypatch
):
    ...
    with pytest.raises(OSError, match="No space left on device"):
        await router._deliver_to_platform(
            target, long_content, metadata={"job_id": "job7"}
        )
