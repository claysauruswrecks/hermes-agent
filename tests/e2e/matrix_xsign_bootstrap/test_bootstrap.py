...
def _register_bot(
    *, prefer_token: str = CONFIG_REG_TOKEN, fallback_token: str | None = None
) -> dict:
...
class XsignBootstrapE2E(unittest.IsolatedAsyncioTestCase):
    ...
    async def _connect_with_bootstrap(
        self, creds: dict, store_dir: Path
        ...
        crypto_db = Database.create(
            f"sqlite:///{db_path}", upgrade_table=PgCryptoStore.upgrade_table
        )
        await crypto_db.start()
        crypto_store = PgCryptoStore(
            account_id=creds["user_id"], pickle_key="e2e-test", db=crypto_db
        )
    ...
    async def asyncSetUp(self):
        self.creds = _register_bot(
            prefer_token=CONFIG_REG_TOKEN, fallback_token=self.first_tok
        )
    ...
    async def _publish_device_keys(self, creds, store_dir):
        ...
        await (
            olm.share_keys()
        )  # publishes device keys (precondition for generate_recovery_key)
        await crypto_db.stop()
        await api.session.close()
    ...
    async def test_bootstrap_publishes_unpadded_keys(self):
        ...
        self.assertTrue(
            any("bootstrapped cross-signing" in l for l in log_lines),
        ...
        self.assertFalse(
            master_kid.endswith("="), f"master keyid is padded: {master_kid!r}"
        )
        self.assertFalse(ssk_kid.endswith("="), f"ssk keyid is padded: {ssk_kid!r}")
    ...
    async def test_second_startup_skips_bootstrap(self):
        ...
        self.assertFalse(
            any("bootstrapped cross-signing" in l for l in log2),
    ...
    async def test_recovery_key_path_takes_precedence(self):
        ...
        try:
            ...
            try:
                ...
                self.assertIsNone(
                    rec2, "bootstrap fired despite MATRIX_RECOVERY_KEY being set"
                )
                self.assertTrue(
