...
class TestIsSafeUrl:
    ...
    def test_qq_multimedia_hostname_allowed_with_benchmark_ip(self):
        with patch(
            ...
            assert (
                is_safe_url("https://multimedia.nt.qq.com.cn/download?id=123") is True
            )
    ...
    def test_qq_multimedia_hostname_exception_is_exact_match(self):
        with patch(
            ...
            assert (
                is_safe_url("https://sub.multimedia.nt.qq.com.cn/download?id=123")
                is False
            )
    ...
    def test_qq_multimedia_hostname_exception_requires_https(self):
        with patch(
            ...
            assert (
                is_safe_url("http://multimedia.nt.qq.com.cn/download?id=123") is False
            )
    ...
    def test_qq_multimedia_hostname_dns_failure_still_blocked(self):
        with patch(
            ...
            assert (
                is_safe_url("https://multimedia.nt.qq.com.cn/download?id=123") is False
            )
...
class TestGlobalAllowPrivateUrls:
    ...
    def test_default_is_false(self, monkeypatch):
        ...
        with patch(
            "hermes_cli.config.read_raw_config", side_effect=Exception("no config")
        ):
