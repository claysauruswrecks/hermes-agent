...
def test_init_env_args_uses_hermes_dotenv_for_empty_shell_env(monkeypatch):
    ...
    monkeypatch.setattr(
        docker_env, "_load_hermes_env_vars", lambda: {"MY_SECRET": "value_from_dotenv"}
    )
...
def test_failed_docker_run_cleans_up_orphaned_container(monkeypatch):
    ...
    assert rm_cmd[3].startswith("hermes-"), (
...
def test_image_uses_init_entrypoint_detects_s6_init(monkeypatch):
    ...
    monkeypatch.setattr(docker_env.subprocess, "run", _run)
    assert (
        docker_env._image_uses_init_entrypoint("/usr/bin/docker", "hermes-agent:latest")
        is True
    )
...
def test_execute_recovers_from_out_of_band_removal(monkeypatch):
    ...
    outputs = iter([
        {
            "output": "Error response from daemon: No such container: hermes-x",
            "returncode": 1,
        },
