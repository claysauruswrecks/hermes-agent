...
class TestSendMessageTool:
    ...
    def test_ntfy_topic_target_bypasses_channel_directory(self):
        ...
        ntfy_cfg = SimpleNamespace(
            enabled=True, token=None, extra={"topic": "hermes-in"}
        )
        ...
        with (
            patch(
                ...
                side_effect=AssertionError(
                    "session_context not consulted when HERMES_KANBAN_TASK is set"
                ),
            ),
            patch(
                "gateway.status.is_gateway_running",
                side_effect=AssertionError(
                    "gateway.status not consulted when HERMES_KANBAN_TASK is set"
                ),
            ),
        ):
            assert _check_send_message() is True
