...
class TestNeedsLightpandaFallback:
    ...
    def test_actual_placeholder_size_triggers_fallback(self, tmp_path):
        ...
        placeholder.write_bytes(
            b"\x89PNG" + b"\x00" * 16693
        )  # actual measured: 16697 bytes
        result = {"success": True, "data": {"path": str(placeholder)}}
        assert _needs_lightpanda_fallback("lightpanda", "screenshot", result) is True
    ...
    def test_unknown_command_does_not_trigger_fallback(self):
        ...
        assert (
            _needs_lightpanda_fallback("lightpanda", "some_future_cmd", result) is False
        )
...
class TestLightpandaFallbackWarning:
    ...
    def test_browser_navigate_surfaces_fallback_warning(self):
        ...
        with (
            ...
            patch(
                ...
                side_effect=[
                    result,
                    {
                        "success": True,
                        "data": {
                            "snapshot": '- heading "Fallback OK" [ref=e1]',
                            "refs": {"e1": {}},
                        },
                    },
                ],
            ...
            response = json.loads(
                bt.browser_navigate("https://example.com", task_id="warn-test")
            )
    ...
    def test_browser_navigate_surfaces_auto_snapshot_fallback_warning(self):
        ...
        with (
            ...
            response = json.loads(
                bt.browser_navigate("https://example.com", task_id="warn-test2")
            )
    ...
    def test_browser_vision_lightpanda_uses_chrome_capture_and_normal_call_llm_shape(
        self, tmp_path
    ):
        import json
        import tools.browser_tool as bt
    ...
    def test_browser_vision_lightpanda_response_has_structured_fallback(self, tmp_path):
        ...
        with (
            ...
            response = json.loads(
                bt.browser_vision("what is this?", task_id="vision-structured")
            )
...
class TestEngineOverride:
    ...
    def test_hybrid_local_sidecar_injects_engine_even_with_cloud_provider(self):
        ...
        with (
            patch(
                "tools.browser_tool._get_session_info",
                return_value={"session_name": "local-sidecar"},
            ),
            patch(
                "tools.browser_tool._find_agent_browser",
                return_value="/usr/bin/agent-browser",
