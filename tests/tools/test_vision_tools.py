...
class TestValidateImageUrl:
    ...
    def test_valid_url_with_query_params(self):
        with patch(
            ...
            assert (
                _validate_image_url("https://img.example.com/pic?w=200&h=200") is True
            )
...
class TestVisionSafetyGuards:
    ...
    async def test_blocked_remote_url_short_circuits_before_download(self):
        ...
        with (
            ...
            patch(
                "tools.vision_tools._download_image", new_callable=AsyncMock
            ) as mock_download,
        ):
            result = json.loads(
                await vision_analyze_tool("https://blocked.test/cat.png", "describe")
            )
...
class TestBase64SizeLimit:
    ...
    async def test_oversized_image_rejected_before_api_call(self, tmp_path):
        ...
        with (
            patch("tools.vision_tools._MAX_BASE64_BYTES", 1000),
            patch(
                "tools.vision_tools.async_call_llm", new_callable=AsyncMock
            ) as mock_llm,
        ):
...
class TestResizeImageForVision:
    ...
    def test_jpeg_output_for_non_png(self, tmp_path):
        ...
        result = _resize_image_for_vision(
            path, mime_type="image/jpeg", max_base64_bytes=50_000
        )
    ...
    def test_extreme_aspect_ratio_preserved(self, tmp_path):
        ...
        result = _resize_image_for_vision(
            path, mime_type="image/png", max_base64_bytes=50_000
        )
        assert result.startswith("data:image/")
...
class TestVisionCpuBurstCap:
    ...
    async def test_encode_runs_on_dedicated_cpu_executor(self):
        ...
        result = await vt._run_encode_on_cpu_executor(
            fake_encode, "p", mime_type="image/jpeg"
        )
    ...
    async def test_encode_bursts_bounded_but_llm_stays_concurrent(self):
        ...
        async def fake_native(image_url, question):
            ...
            try:
                ...
                await vt._run_encode_on_cpu_executor(
                    slow_encode, "p", mime_type="image/jpeg"
                )
