...
class TestFindDocker:
    def test_found_via_shutil_which(self):
        with patch(
            "tools.environments.docker.shutil.which", return_value="/usr/bin/docker"
        ):
    ...
    def test_returns_none_when_not_found(self):
        with (
            patch("tools.environments.docker.shutil.which", return_value=None),
            patch(
                "tools.environments.docker._DOCKER_SEARCH_PATHS",
                ["/nonexistent/docker"],
            ),
        ):
            result = docker_mod.find_docker()
    ...
    def test_caches_result(self):
        with patch(
            "tools.environments.docker.shutil.which",
    ...
    def test_env_var_override_takes_precedence(self, tmp_path):
        ...
        with (
            patch.dict(os.environ, {"HERMES_DOCKER_BINARY": str(fake_binary)}),
            patch(
                "tools.environments.docker.shutil.which", return_value="/usr/bin/docker"
            ),
        ):
    ...
    def test_env_var_override_ignored_if_nonexistent(self):
        ...
        with (
            patch.dict(os.environ, {"HERMES_DOCKER_BINARY": "/nonexistent/podman"}),
            patch(
                "tools.environments.docker.shutil.which", return_value="/usr/bin/docker"
            ),
        ):
    ...
    def test_podman_on_path_used_when_docker_missing(self):
        ...
        with (
            patch(
                "tools.environments.docker.shutil.which", side_effect=which_side_effect
            ),
