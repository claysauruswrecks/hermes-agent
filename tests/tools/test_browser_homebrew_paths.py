...
class TestBrowserRequirements:
    def test_cdp_override_does_not_require_agent_browser_cli(self, monkeypatch):
        monkeypatch.setenv(
            "BROWSER_CDP_URL", "ws://127.0.0.1:9222/devtools/browser/test"
        )
    ...
    def test_termux_requires_real_agent_browser_install_not_npx_fallback(
        self, monkeypatch
    ):
        ...
        monkeypatch.setattr(
            "tools.browser_tool._find_agent_browser", lambda **_kw: "npx agent-browser"
        )
...
class TestRunBrowserCommandPathConstruction:
    ...
    def test_subprocess_preserves_executable_path_with_spaces(self, tmp_path):
        ...
        with (
            patch("tools.browser_tool._find_agent_browser", return_value=browser_path),
    ...
    def test_subprocess_path_includes_homebrew_node_dirs(self, tmp_path):
        ...
        with (
            ...
            patch(
                "tools.browser_tool._discover_homebrew_node_dirs",
                return_value=fake_homebrew_dirs,
            ),
            patch("os.path.isdir", side_effect=selective_isdir),
            patch("subprocess.Popen", side_effect=capture_popen),
            patch("os.open", return_value=99),
            ...
            patch("tools.interrupt.is_interrupted", return_value=False),
            patch.dict(
                os.environ, {"PATH": "/usr/bin:/bin", "HOME": "/home/test"}, clear=True
            ),
        ):
