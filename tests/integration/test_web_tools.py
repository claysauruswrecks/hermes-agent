...
class WebToolsTester:
    ...
    def log_result(self, test_name: str, status: str, details: str = ""):
        ...
        if status == "passed":
            ...
            print_warning(
                f"{test_name} skipped: {details}" if details else f"{test_name} skipped"
            )
    ...
    def test_environment(self) -> bool:
        ...
        if not check_web_api_key():
            self.log_result(
                "Web Backend API Key",
                "failed",
                "PARALLEL_API_KEY or FIRECRAWL_API_KEY not set",
            )
            return False
        else:
        ...
        self.log_result(
            "Auxiliary LLM",
            "skipped",
            "web_extract no longer uses an LLM (truncate-and-store)",
        )
        self.test_llm = False
    ...
    def test_web_search(self) -> List[str]:
        ...
        test_queries = [
            ...
            (
                "inflammatory arthritis symptoms treatment",
                8,
            ),  # Test medical query from your example
        ]
        ...
        for query, limit in test_queries:
            try:
                ...
                try:
                    ...
                    self.log_result(
                        f"Search: {query[:30]}...", "failed", f"Invalid JSON: {e}"
                    )
                    if self.verbose:
                ...
                if not web_results:
                    self.log_result(
                        f"Search: {query[:30]}...", "failed", "Empty web results array"
                    )
                    if self.verbose:
                        print(
                            f"    data.web content: {data.get('data', {}).get('web')}"
                        )
                    continue
    ...
    async def test_web_extract(self, urls: List[str] = None):
        ...
        if urls:
            try:
                ...
                if self.verbose:
                    print(
                        f"  Calling web_extract_tool(urls={test_urls}, format='markdown')"
                    )
                ...
                if "error" in data:
                    self.log_result(
                        "Extract (no LLM)", "failed", f"API error: {data['error']}"
                    )
                    return
                ...
                if not results:
                    self.log_result(
                        "Extract (no LLM)", "failed", "No results in response"
                    )
                    if self.verbose:
                        print(f"    Response keys: {list(data.keys())}")
                    return
                ...
                for i, result in enumerate(results):
                    ...
                    if error:
                        ...
                        extraction_details.append(
                            f"Page {i + 1}: {title[:40]}... ({content_len} chars)"
                        )
                        if self.verbose:
                            print(
                                f"    Page {i + 1}: ✓ {title[:50]}... - {content_len} characters"
                            )
                            print(f"      First 100 chars: {content[:100]}...")
                    else:
                        extraction_details.append(
                            f"Page {i + 1}: {title[:40]}... (EMPTY)"
                        )
                        if self.verbose:
                            print(
                                f"    Page {i + 1}: ⚠ {title[:50]}... - Empty content"
                            )
        ...
        try:
            ...
            if content:
                ...
                if content_len > 0:
                    ...
                    self.log_result(
                        "Extract (with LLM)", "failed", "No content after processing"
                    )
            else:
                self.log_result(
                    "Extract (with LLM)", "failed", "No content field in result"
                )
    ...
    async def run_all_tests(self):
        ...
        print(
            f"{Colors.WARNING}Skipped: {len(self.test_results['skipped'])}{Colors.ENDC}"
        )
...
async def main():
    ...
    parser.add_argument(
        "--no-llm", action="store_true", help="Skip LLM processing tests"
    )
    parser.add_argument(
        "--verbose", "-v", action="store_true", help="Show detailed output"
    )
    parser.add_argument(
        "--debug", action="store_true", help="Enable debug mode for web tools"
    )
    ...
    # Create tester
    tester = WebToolsTester(verbose=args.verbose, test_llm=not args.no_llm)
