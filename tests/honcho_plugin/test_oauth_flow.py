...
def test_config_path_rides_the_authorize_link(fake_as):
    ...
    url, _ = oauth_flow.begin_authorization(
        endpoints, config_path="~/.hermes/honcho.json"
    )
...
def test_display_config_path_never_leaks_absolute_path():
    ...
    assert (
        ...
        == "~/.hermes/profiles/work/honcho.json"
    )
    assert (
...
def test_launcher_runs_flow_in_background_and_reports_connected(
    ...
    st = oauth_flow.start_loopback_flow_background(
        config_path=Path("/t/honcho.json"), host="hermes"
    )
...
def test_launcher_is_idempotent_while_pending(monkeypatch, reset_flow):
    ...
    s1 = oauth_flow.start_loopback_flow_background(
        config_path=Path("/t/h.json"), host="hermes"
    )
