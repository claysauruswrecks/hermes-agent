...
def test_model_flow_nous_does_not_restore_stale_custom_api_key(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "hermes_cli.models.get_pricing_for_provider", lambda provider: {}
    )
    monkeypatch.setattr(
        "hermes_cli.models.check_nous_free_tier", lambda **kwargs: False
    )
    monkeypatch.setattr(
...
def test_model_flow_openrouter_clears_stale_custom_key(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "hermes_cli.models.get_pricing_for_provider", lambda *a, **k: {}
    )
    monkeypatch.setattr(
        "hermes_cli.auth._prompt_model_selection",
...
def test_model_flow_anthropic_clears_stale_custom_key_and_mode(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "hermes_cli.auth.get_anthropic_key", lambda: "sk-ant-api03-test"
    )
    monkeypatch.setattr(
...
def test_model_flow_nous_offers_tool_gateway_prompt_when_unconfigured(
    ...
    monkeypatch.setattr(
        "hermes_cli.setup.prompt_checklist", _fake_checklist, raising=False
    )
    ...
    assert (
        "Tool Gateway" in captured["title"] or "tool pool" in captured["title"].lower()
    )
...
def test_model_flow_custom_persists_selected_api_mode(monkeypatch):
    ...
    monkeypatch.setattr(
        "hermes_cli.config.save_config", lambda cfg: saved_cfg.update(cfg)
    )
    monkeypatch.setattr(
        "hermes_cli.main._save_custom_provider",
        lambda base_url, api_key="", model="", context_length=None, name=None, api_mode=None: (
