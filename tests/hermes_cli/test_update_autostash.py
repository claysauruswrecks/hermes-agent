...
def test_resolve_stash_selector_returns_matching_entry(monkeypatch, tmp_path):
    ...
    assert (
        hermes_main._resolve_stash_selector(["git"], tmp_path, "abc123") == "stash@{1}"
    )
...
def _setup_update_mocks(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        hermes_main, "_stash_local_changes_if_needed", lambda *a, **kw: None
    )
    monkeypatch.setattr(hermes_main, "_restore_stashed_changes", lambda *a, **kw: True)
    monkeypatch.setattr(
        hermes_config, "get_missing_env_vars", lambda required_only=True: []
    )
