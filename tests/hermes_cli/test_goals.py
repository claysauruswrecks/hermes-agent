...
class TestParseJudgeResponse:
    def test_clean_json_done(self):
        ...
        verdict, reason, _pf, wait = _parse_judge_response(
...
class TestJudgeParseFailureAutoPause:
    ...
    def test_auto_pause_after_three_consecutive_parse_failures(self, hermes_home):
        ...
        with patch.object(
            goals,
            "judge_goal",
            return_value=("continue", "judge returned empty response", True, None),
        ):
            d1 = mgr.evaluate_after_turn("step 1")
            assert d1["should_continue"] is True
    ...
    def test_parse_failure_counter_resets_on_good_reply(self, hermes_home):
        ...
        with patch.object(
            goals,
            "judge_goal",
            return_value=("continue", "making progress", False, None),
        ):
    ...
    def test_parse_failure_counter_not_incremented_by_api_errors(self, hermes_home):
        ...
        with patch.object(
            goals,
            "judge_goal",
            return_value=("continue", "judge error: RuntimeError", False, None),
        ):
            for _ in range(5):
...
class TestMigrateGoalToSession:
    ...
    def test_migrates_active_goal_to_child(self, hermes_home):
        ...
        assert (
            migrate_goal_to_session("parent-sid", "child-sid", reason="compression")
            is True
        )
...
class TestWaitBarrier:
    ...
    def test_barrier_auto_clears_when_process_exits_and_loop_resumes(self, hermes_home):
        ...
        with patch.object(
            goals, "judge_goal", return_value=("continue", "more", False, None)
        ):
    ...
    def test_dead_pid_never_parks(self, hermes_home):
        ...
        with patch.object(
            goals, "judge_goal", return_value=("continue", "go", False, None)
        ):
    ...
    def test_continue_verdict_still_continues_with_background(self, hermes_home):
        ...
        with patch.object(
            ...
            decision = mgr.evaluate_after_turn(
                "made progress",
                background_processes=[
                    {"pid": 999999, "command": "x", "status": "running"}
                ],
            )
        assert decision["verdict"] == "continue"
        assert decision["should_continue"] is True
...
class TestSessionTriggerBarrier:
    ...
    def test_goal_parks_on_session_and_releases_on_trigger(self, hermes_home):
        ...
        with patch.object(
            goals, "judge_goal", return_value=("continue", "build done", False, None)
        ):
        ...
        bg = [
            {
                ...
                "pid": 4242,
                "status": "running",
                "command": "wait_for_pr_green.sh 50501",
                "trigger": "exit",
            }
        ]
        with (
...
# ──────────────────────────────────────────────────────────────────────
# Goal-judge timeout configuration tests
# ──────────────────────────────────────────────────────────────────────


class TestJudgeGoalTimeoutParameter:
    """Tests for judge_goal timeout parameter handling."""

    def test_judge_goal_accepts_timeout_parameter(self, hermes_home):
        """Test that judge_goal accepts an explicit timeout parameter."""
        from hermes_cli import goals

        # Verify the function signature accepts timeout=None or a float
        import inspect

        sig = inspect.signature(goals.judge_goal)
        assert "timeout" in sig.parameters
        param = sig.parameters["timeout"]
        # Should be Optional[float] with default None
        assert param.default is None

    def test_draft_contract_accepts_timeout_parameter(self, hermes_home):
        """Test that draft_contract accepts an explicit timeout parameter."""
        from hermes_cli import goals

        # Verify the function signature accepts timeout=None or a float
        import inspect

        sig = inspect.signature(goals.draft_contract)
        assert "timeout" in sig.parameters
        param = sig.parameters["timeout"]
        # Should be Optional[float] with default None
        assert param.default is None
...
class TestGoalJudgeTimeoutConfig:
    """Tests for goal_judge timeout config resolution."""

    def test_goal_judge_timeout_from_config(self, hermes_home, monkeypatch):
        """Test that _goal_judge_timeout reads from auxiliary.goal_judge.timeout in config."""
        from hermes_cli import goals

        # Mock load_config to return custom timeout
        mock_cfg = {
            "auxiliary": {
                "goal_judge": {
                    "provider": "auto",
        ...
        with monkeypatch.context() as m:
            ...
            original_load = getattr(cfg_mod, "load_config", None)

            def mock_load():
                return mock_cfg

            m.setattr(cfg_mod, "load_config", mock_load)

            # Call _goal_judge_timeout
            timeout_val = goals._goal_judge_timeout()
            assert timeout_val == 120.0

    def test_goal_judge_timeout_fallback_to_default(self):
        """Test that _goal_judge_timeout falls back to DEFAULT_JUDGE_TIMEOUT when config is missing."""
        from hermes_cli import goals

        with patch(
            "hermes_cli.config.load_config", side_effect=Exception("config error")
        ):
            timeout_val = goals._goal_judge_timeout()
            assert timeout_val == goals.DEFAULT_JUDGE_TIMEOUT
