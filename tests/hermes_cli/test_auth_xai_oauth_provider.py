...
def test_xai_callback_server_accepts_fallback_code_while_browser_connection_is_stuck():
    ...
    stuck = socket.create_connection(
        (XAI_OAUTH_REDIRECT_HOST, server.server_address[1]), timeout=2
    )
    try:
...
def test_xai_callback_server_latches_first_terminal_callback_result():
    ...
    try:
        with urllib.request.urlopen(
            f"{redirect_uri}?code=first-code&state=state-1", timeout=2
        ) as response:
            assert response.status == 200
        with urllib.request.urlopen(
...
def test_resolve_credentials_quarantines_dead_tokens_on_terminal_refresh_failure(
    ...
    _seed_xai_oauth_state(
        hermes_home, dict(_STALE_XAI_OAUTH_STATE), active_provider="nous"
    )
    monkeypatch.setenv("HERMES_HOME", str(hermes_home))
...
def test_pool_seeded_entry_sync_back_after_refresh(tmp_path, monkeypatch):
    ...
    near_expiry = _jwt_with_exp(int(time.time()) + 30)
    _setup_hermes_auth(
        hermes_home, access_token=near_expiry, refresh_token="rt-singleton"
    )
    monkeypatch.setenv("HERMES_HOME", str(hermes_home))
...
def test_pool_refresh_recovers_when_other_process_already_refreshed(
    ...
    _setup_hermes_auth(
        hermes_home, access_token=in_memory_at, refresh_token="rt-shared"
    )
    monkeypatch.setenv("HERMES_HOME", str(hermes_home))
...
def test_pool_manual_xai_entry_not_synced_from_singleton(tmp_path, monkeypatch):
    ...
    _setup_hermes_auth(
        hermes_home, access_token=singleton_at, refresh_token="rt-singleton"
    )
    monkeypatch.setenv("HERMES_HOME", str(hermes_home))
