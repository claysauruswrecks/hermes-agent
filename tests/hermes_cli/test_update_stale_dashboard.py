    ...
    def test_matches_running_dashboard(self):
        with patch("subprocess.run") as mock_run:
            mock_run.return_value = MagicMock(
                returncode=0,
                stdout=_ps_line(
                    12345, "python3 -m hermes_cli.main dashboard --port 9119"
                )
                + "\n",
                stderr="",
            )
...
class TestKillStaleDashboardPosix:
    ...
    def test_sigkill_fallback_for_survivors(self, capsys):
        ...
        with (
            patch("hermes_cli.main._find_stale_dashboard_pids", return_value=[99999]),
            patch("os.kill", side_effect=fake_kill),
            patch("time.sleep"),
    ...
    def test_permission_error_is_reported_not_raised(self, capsys):
        ...
        with (
            patch("hermes_cli.main._find_stale_dashboard_pids", return_value=[12345]),
            patch("os.kill", side_effect=fake_kill),
            patch("time.sleep"),
        ):
