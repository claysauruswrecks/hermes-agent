...
def test_ensure_direct_aliases_mutates_in_place(monkeypatch):
    ...
    mock_data = {
        "my-custom-alias": DirectAlias(
            "custom-model:v1", "custom", "https://example.com/v1"
        )
    }
    ...
    assert id(ms.DIRECT_ALIASES) == initial_id, (
        f"DIRECT_ALIASES was rebound (ID changed from {initial_id} to {id(ms.DIRECT_ALIASES)})"
    )
...
def test_resolve_named_custom_runtime_honors_explicit_base_url(monkeypatch):
    ...
    monkeypatch.setattr(
        "hermes_cli.runtime_provider.has_usable_secret", lambda x: x == "test-api-key"
    )
