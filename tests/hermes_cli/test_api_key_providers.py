...
class TestApiKeyProviderStatus:
    ...
    def test_copilot_status_uses_gh_cli_token(self, monkeypatch):
        monkeypatch.setattr(
            "hermes_cli.copilot_auth._try_gh_cli_token", lambda: "gho_gh_cli_token"
        )
    ...
    def test_get_auth_status_dispatches_to_external_process(self, monkeypatch):
        monkeypatch.setattr(
            "hermes_cli.auth.shutil.which", lambda command: f"/opt/bin/{command}"
        )
...
class TestResolveApiKeyProviderCredentials:
    def test_resolve_zai_with_key(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "hermes_cli.auth.detect_zai_endpoint", lambda *a, **kw: None
        )
    ...
    def test_resolve_copilot_with_gh_cli_fallback(self, monkeypatch):
        monkeypatch.setattr(
            "hermes_cli.copilot_auth._try_gh_cli_token", lambda: "gho_cli_secret"
        )
        creds = resolve_api_key_provider_credentials("copilot")
        assert creds["provider"] == "copilot"
    ...
    def test_resolve_lmstudio_normalizes_native_api_base_url_from_env(
        self, monkeypatch
    ):
        monkeypatch.setenv("LM_API_KEY", "lm-token")
    ...
    def test_try_gh_cli_token_uses_homebrew_path_when_not_on_path(self, monkeypatch):
        monkeypatch.setattr(
            "hermes_cli.copilot_auth.shutil.which", lambda command: None
        )
        monkeypatch.setattr(
...
class TestHasAnyProviderConfigured:
    ...
    def test_claude_code_creds_ignored_on_fresh_install(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr(config_module, "get_hermes_home", lambda: hermes_home)
        monkeypatch.setattr(
            "hermes_cli.copilot_auth.resolve_copilot_token", lambda: ("", "")
        )
    ...
    def test_claude_code_creds_counted_when_hermes_configured(
