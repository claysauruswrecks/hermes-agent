...
def test_gui_installs_packages_and_launches_desktop_app(tmp_path, monkeypatch):
    ...
    with (
        ...
        patch(
            "hermes_cli.main._run_npm_install_deterministic", return_value=install_ok
        ) as mock_install,
        patch("hermes_cli.main._desktop_build_needed", return_value=True),
        patch("hermes_cli.main._write_desktop_build_stamp"),
        patch("hermes_cli.main._desktop_macos_relaunchable_fixup"),
        patch(
            "hermes_cli.main.subprocess.run", side_effect=[pack_ok, launch_ok]
        ) as mock_run,
...
def test_gui_source_mode_uses_renderer_build_and_electron(tmp_path, monkeypatch):
    ...
    with (
        ...
        patch("hermes_cli.main._write_desktop_build_stamp"),
        patch(
            "hermes_cli.main.subprocess.run", side_effect=[build_ok, launch_ok]
        ) as mock_run,
        pytest.raises(SystemExit) as exc,
    ):
...
def test_gui_retries_pack_once_after_purging_build_cache(tmp_path, monkeypatch):
    ...
    with (
        ...
        patch("hermes_cli.main._write_desktop_build_stamp"),
        patch(
            "hermes_cli.main._purge_electron_build_cache",
        ...
        patch(
            "hermes_cli.main.subprocess.run", side_effect=run_side_effect
        ) as mock_run,
        pytest.raises(SystemExit) as exc,
    ):
        cli_main.cmd_gui(_ns())
...
def test_gui_redownloads_electron_via_mirror_then_repacks(
    ...
    with (
        ...
        patch("hermes_cli.main._electron_dist_ok", return_value=False),
        patch(
            "hermes_cli.main._redownload_electron_dist", side_effect=[False, True]
        ) as mock_dl,
        patch(
            "hermes_cli.main.subprocess.run", side_effect=[pack_fail, pack_fail]
        ) as mock_run,
        pytest.raises(SystemExit) as exc,
    ):
        cli_main.cmd_gui(_ns())
...
def test_gui_install_failure_self_heals_electron_and_continues(
    ...
    with (
        patch("hermes_cli.main.shutil.which", return_value="/usr/bin/npm"),
        patch(
            "hermes_cli.main._run_npm_install_deterministic", return_value=install_fail
        ),
        patch("hermes_cli.main._desktop_linux_sandbox_fixup", return_value=True),
        patch("hermes_cli.main._write_desktop_build_stamp"),
        patch("hermes_cli.main._electron_dist_ok", return_value=False),
        patch(
            "hermes_cli.main._try_redownload_electron_dist", return_value=True
        ) as mock_dl,
        patch(
            "hermes_cli.main.subprocess.run", side_effect=[pack_ok, launch_ok]
        ) as mock_run,
...
def test_gui_does_not_retry_after_packaged_executable_exists(
    ...
    with (
        ...
        patch(
            "hermes_cli.main._redownload_electron_dist", return_value=True
        ) as mock_dl,
...
def test_gui_does_not_override_user_electron_mirror(tmp_path, monkeypatch, capsys):
    ...
    with (
        ...
        patch("hermes_cli.main._desktop_macos_relaunchable_fixup"),
        patch(
            "hermes_cli.main._purge_electron_build_cache", return_value=[]
        ) as mock_purge,
...
def test_redownload_electron_dist_missing_installer(tmp_path, monkeypatch):
    ...
    with (
        patch("hermes_cli.main.shutil.which", return_value="/usr/bin/node"),
        patch("hermes_cli.main.subprocess.run") as mock_run,
    ):
