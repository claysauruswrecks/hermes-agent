...
class TestBuildWebUISkipsWhenFresh:
    ...
    def test_runs_npm_when_dist_missing(self, tmp_path):
        ...
        with (
            ...
            patch(
                "hermes_cli.main._run_with_idle_timeout", return_value=build_ok
            ) as mock_idle,
        ):
            result = _build_web_ui(web_dir)
    ...
    def test_web_install_omits_workspace_when_web_has_own_lockfile(
        ...
        with (
            patch("hermes_cli.main.shutil.which", return_value="/usr/bin/npm"),
            patch(
                "hermes_cli.main.subprocess.run", return_value=install_cp
            ) as mock_run,
    ...
    def test_web_build_uses_idle_timeout_helper(self, tmp_path):
        ...
        with (
            ...
            patch(
                "hermes_cli.main._run_with_idle_timeout", return_value=build_cp
            ) as mock_idle,
        ):
            result = _build_web_ui(web_dir)
