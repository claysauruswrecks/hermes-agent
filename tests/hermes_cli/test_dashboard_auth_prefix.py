...
class TestPublicUrlOverride:
    ...
    def test_scheme_less_public_url_warning_is_deduplicated(
        ...
        scheme_warnings = [
            ...
            if r.levelno == logging.WARNING and "hermes.domain.com" in r.getMessage()
        ]
    ...
    def test_valid_public_url_emits_no_warning(self, patch_config, monkeypatch, caplog):
        ...
        prefix_mod._warned_malformed_public_urls.clear()
        patch_config(None)
        monkeypatch.setenv("HERMES_DASHBOARD_PUBLIC_URL", "https://hermes.domain.com")
...
class TestCookiePathRespectsPrefix:
    ...
    def test_pkce_cookie_uses_prefix_path(self, gated_app_proxied):
        ...
        # to /billing/... etc.
        assert "Path=/hermes" in pkce, f"PKCE cookie has wrong Path: {pkce!r}"
    ...
    def test_pkce_cookie_uses_secure_prefix_when_proxied(self, gated_app_proxied):
        ...
        pkce_candidates = [
            c for c in cookies if c.startswith("__Secure-hermes_session_pkce=")
        ]
    ...
    def test_pkce_cookie_uses_host_prefix_when_direct(self, gated_app_direct):
        ...
        cookies = r.headers.get_list("set-cookie")
        pkce_candidates = [
            c for c in cookies if c.startswith("__Host-hermes_session_pkce=")
        ]
    ...
    def test_cookies_read_back_round_trip_through_prefix(self, gated_app_proxied):
        ...
        pkce_set = next(
            c for c in r1.headers.get_list("set-cookie") if "hermes_session_pkce" in c
        )
        ...
        at_cookies = [c for c in cookies if c.startswith("__Secure-hermes_session_at=")]
