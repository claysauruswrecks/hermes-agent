...
def test_first_install_nous_auto_configures_managed_defaults(monkeypatch):
    monkeypatch.setattr(
        "hermes_cli.nous_subscription.managed_nous_tools_enabled", lambda: True
    )
    config = {
        "model": {"provider": "nous"},
        "platform_toolsets": {"cli": []},
...
def test_get_platform_tools_recovers_non_configurable_toolsets_from_composite():
    ...
    with mock_patch(
        "hermes_cli.tools_config.PLATFORMS", {**PLATFORMS, **test_platforms}
    ):
...
def test_reconfigure_provider_runs_post_setup_for_env_var_providers(
    ...
    called = []
    monkeypatch.setattr(
        "hermes_cli.tools_config._run_post_setup", lambda key: called.append(key)
    )
...
def test_configure_non_managed_provider_skips_portal_gate(monkeypatch):
    ...
    monkeypatch.setattr("hermes_cli.nous_subscription.ensure_nous_portal_access", _boom)
